...
export default async function handler(
  ...
  try {
    const { recipientEmail, recipientName, analysis, input } = req.body as EmailRequestBody;

    // Validate required fields
    if (!recipientEmail || !recipientName || !analysis || !input) {
      return res.status(400).json({
        error: 'Missing required fields: recipientEmail, recipientName, analysis, or input'
      });
    }
